...
class ArchetypalAnalysis1():
    ...
    initialize:     Defines the initialization method to guess initial archetypes:
                        ...
                        4. furthest_first: Choose 1 initial point randomly and select the second point as the data point farthest away from the first. The code was adapted from https://github.com/smair/archetypalanalysis-coreset/tree/master
                        5. aa_coreset : Choose point based on Coreset algorithm. The code was adapted from https://github.com/smair/archetypalanalysis-coreset/tree/master
                        6. aa_pp : sequentially samples points based on their influence on the objective function (from Archetypal Analysis++). The code was adapted from https://github.com/smair/archetypalanalysis-initialization/tree/main
                        7. k_means_pp : approximate the distance computation used by K-means++. The code was adapted from https://github.com/smair/archetypalanalysis-initialization/tree/main
...
#        self.alfa, self.RSSi_ = self.__optimize_alfa_for_transform(X_new, n_data)
    ...
    def _initialize_archetypes(self):
        ...
        elif self.initialize == 'furthest_first':
            self._furthest_first_initialize()

        elif self.initialize == 'aa_coreset':
            self._aa_coreset_initialize()

        elif self.initialize == 'aa_pp':
            self._aa_pp_initialize()

        elif self.initialize == 'k_means_pp':
            self._kmeans_pp_initialize()
    ...
    def _furthest_sum_initialize(self):
        np.random.seed(self.random_state)   #with random seed to make the archetype the same if using the same randomstate
    ...
    def _furthest_first_initialize(self):
        n = self.X.shape[1]
        ind = []
        k = self.n_archetypes
        Xt = self.X.T
        ...
        #i = np.random.choice(n, 1).item()   # without random seed will create different archetype each run time

        # 2 lines below use random seed that will create same archetype each run time. If you want to change the archetype, just use different randomstate
        rng = np.random.default_rng(self.random_state)  # Set seed for reproducibility
        i = rng.choice(n, 1).item()

        ind.append(i)
    
        # sample remaining points
        for _ in range(k-1):
            dist = np.array([np.linalg.norm(Xt-Xt[i], ord=2, axis=1) for i in ind])
        ...
        # Because ind is a list of index, we need the find the respective data of that list
        data_array = self.X
        
        # Define the column indices to extract
        column_indices = copy(ind)
        
        # Ensure selected column indices exist
        if max(column_indices) >= data_array.shape[1]:
            raise ValueError("One or more column indices exceed the number of available columns in the dataset.")
        
        # Extract the required columns as a 2D NumPy array (52 rows, 5 columns)
        #extracted_data = data_array[:52, column_indices]

        # Extract the required columns as a 2D NumPy array (104 rows, 5 columns) for left combine right
        extracted_data = data_array[:104, column_indices]

        self.archetypes = copy(extracted_data)

    def _aa_coreset_initialize(self):
        ...
        n = Xa.shape[0]
        dist = np.sum((Xa-Xa.mean(axis=0))**2, axis=1)
        q = dist / dist.sum()
        #ind = np.random.choice(n, k, p=q)   # without random seed will create different archetype each run time
        
        # 2 lines below use random seed that will create same archetype each run time. If you want to change the archetype, just use different randomstate
        rng = np.random.default_rng(self.random_state)  # Local RNG with optional seed
        ind = rng.choice(n, k, p=q, replace=False)
    ...
    def _aa_pp_initialize(self):
        ...
        # sample second point
        dist = np.sum((Xa-Xa[i])**2, axis=1)
        #i = np.random.choice(n, 1, p=dist/dist.sum()).item()   # without random seed will create different archetype each run time
        # 1 line below use rng previously set with random seed that will create same archetype each run time. 
        i = rng.choice(n, 1, p=dist/dist.sum()).item()
        ind.append(i)
        
        # sample remaining points
        for _ in range(k-2):
            A = self.ArchetypalAnalysis_compute_A(Xa, Xa[ind])
            dist = np.sum((Xa-A@Xa[ind])**2, axis=1)
            #i = np.random.choice(n, 1, p=dist/dist.sum()).item()   # without random seed will create different archetype each run time
            # 1 line below use rng previously set with random seed that will create same archetype each run time. 
            i = rng.choice(n, 1, p=dist/dist.sum()).item()
            ind.append(i)
    ...
    def _kmeans_pp_initialize(self):
        Xa = self.X.T
        k = self.n_archetypes
        n = Xa.shape[0]
        ind = []
        ...
        # sample remaining points
        for _ in range(k-1):
            dist = np.array([np.sum((Xa-Xa[i])**2, axis=1) for i in ind])
            closest_cluster_id = dist.argmin(0)
            dist = dist[closest_cluster_id, np.arange(n)]
            #i = np.random.choice(n, 1, p=dist/dist.sum()).item()   # without random seed will create different archetype each run time
            # 1 line below use rng previously set with random seed that will create same archetype each run time.
    ...
    def ArchetypalAnalysis_compute_A(AA, Xa, Za, M=1000.0):
        # initialization
        n = Xa.shape[0]
        k = Za.shape[0]
        A = np.zeros((n, k))
        ...
        Q = np.vstack((Za.T, M * np.ones(k)))
        for i in range(n):
            ai, rnorm = nnls(Q, np.hstack((Xa[i], M)))
            A[i] = ai.T
...
#        self.exp_var_per_arch_ = tuple(self.exp_var_per_arch_)
    ...
    def plot_simplex(self, alfa, plot_args = {}, grid_on = True, ffigure = "plot_alfa"):
    ...
    def _extract_closes_match_aapp(self, X, archetyp, alfa):
        
        from scipy.spatial.distance import cdist
        
        self.close_match = {}
        for i in range(self.n_archetypes):
            r = cdist(X, archetyp[:,i].reshape(1, self.n_dim))
            i_min = np.argmin(r)
            
            self.close_match[i+1] = (i_min, alfa[:,i_min])
...
def create_simplex_ax(AA, grid_on = True, gridcolor = '#EAECEE',
